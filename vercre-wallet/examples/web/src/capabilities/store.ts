...
const hardCoded = (): Seq<uint8> => {
    ...
    let serializer = new BincodeSerializer();
    c0.serialize(serializer);
    let b0 = serializer.getBytes();

    serializer = new BincodeSerializer();
    c1.serialize(serializer);
    let b1 = serializer.getBytes();

    // let list = new Uint8Array([ b0, b1 ]);

    // let bytes: number[] = JSON.parse(JSON.stringify(list));
    return Array.from(b0).concat(Array.from(b1));
};
...
const list = async (): Promise<StoreResponse> => {
    ...
    console.log('hardCoded output:', list);
