...
mod test {
    ...
    #[test]
    fn proof_err() {
        let err: Error = Err::InvalidProof("c_nonce".to_string(), 10).into();

        assert_eq!(err.c_nonce(), Some(("c_nonce".to_string(), 10)));
        assert_eq!(
            err.to_json(),
            json!({
                "error": "invalid_proof",
    ...
    fn test_from_err() {
        ...
        assert_eq!(error.to_string(), "server_error".to_string());
