...
pub async fn authorize(
    ...
    let mut issuance = match &request.initiator {
            ...
            match StateStore::get::<IssuanceState>(&provider, issuance_id).await {
                Ok(issuance) => {
                    ...
                    if let Some(grants) = &offer.grants.clone() {
                        ...
                        let Some(grant_types) = &authorization_server.oauth.grant_types_supported
                        else {
        ...
        Initiator::Wallet {
            ...
            // TODO: Temporary hack until all moved over to state-mutation flow.
            let flow = FlowType::HolderScope;
