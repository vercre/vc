...
Vercre's support for platforms and features can be distinguished with three
...
additionally tailored for Vercre. Vercre's tiered support additionally
applies not only to platforms/targets themselves but additionally features
implemented within Vercre itself.

The purpose of this document is to provide a means by which to evaluate the
inclusion of new features and support for existing features within Vercre.
This should not be used to "lawyer" a change into Vercre on a precise
...
[`example proposal`]: https://github.com/vercre/vercre/blob/master/proposals/example-proposal/Overview.md
...
[^1]: This is intended to encompass features that Vercre supports as a
general-purpose code generator such as integer value types other than `i32` and
`i64`, non-Vercre calling conventions, code model settings, relocation
restrictions, etc. These features aren't covered by Vercre's usage of
Vercre because the WebAssembly instruction set doesn't leverage them. This
means that they receive far less testing and fuzzing than the parts of Vercre
exercised by Vercre.
...
[^3]: Vercre's `vercre` and `winch` features can be compiled to WebAssembly
but not the `runtime` feature at this time. This means that
Vercre-compiled-to-wasm can itself compile wasm but cannot execute wasm.
...
Vercre's precise definitions of tiers are not guaranteed to be constant over
