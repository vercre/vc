...
pub(in crate::model) fn serialize<T, S>(value: &[T], serializer: S) -> Result<S::Ok, S::Error>
where
    T: Serialize,
    S: Serializer,
{
...
pub(in crate::model) fn deserialize<'de, T, D>(deserializer: D) -> Result<Vec<T>, D::Error>
where
...
mod tests {
    ...
    fn test_flex() {
        let test_data = TestData {
            string: "string".into(),
            object: vec![Nested { n: "object".into() }],
            object_array: vec![Nested { n: "object1".into() }, Nested { n: "object2".into() }],
            array: vec!["item1".into(), "item2".into()],
