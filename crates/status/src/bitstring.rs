...
pub fn validate(_resolver: &impl verifier::Status, _status: &CredentialStatus) -> Result<bool, Error> {
    ...
    // Let status purpose be the value of statusPurpose in the credentialStatus entry in the credentialToValidate.
    // Dereference the statusListCredential URL, and ensure that all proofs verify successfully. If the dereference fails, raise a STATUS_RETRIEVAL_ERROR. If any of the proof verifications fail, raise a STATUS_VERIFICATION_ERROR.
    // Verify that the status purpose is equal to a statusPurpose value in the statusListCredential. Note: The statusListCredential might contain multiple status purposes in a single list. If the values are not equal, raise a STATUS_VERIFICATION_ERROR.
    // Let compressed bitstring be the value of the encodedList property of the BitstringStatusListCredential.
    // Let credentialIndex be the value of the statusListIndex property of the BitstringStatusListEntry.
    // Generate a revocation bitstring by passing compressed bitstring to the Bitstring Expansion Algorithm.
    // If the length of the revocation bitstring divided by statusSize is less than minimumNumberOfEntries, raise a STATUS_LIST_LENGTH_ERROR.
    // Let status be the value in the bitstring at the position indicated by the credentialIndex multiplied by the size. If the credentialIndex multiplied by the size is a value outside of the range of the bitstring, a RANGE_ERROR MUST be raised.
    ...
    // If status is 0, set the valid key in result to true; otherwise, set it to false.
    // If the statusPurpose is message, set the message key in result to the corresponding message of the value as indicated in the statusMessages array.
    // Return result.
